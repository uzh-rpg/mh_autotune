...
class Quadrotor():
  def __init__(self, z_offset, L_pivots, resources_path, file_name, parameters_path, node_name):
    self.z_offset = z_offset
    self.L_pivots = L_pivots
    self.resources_path = resources_path
    self.file_name = file_name
    self.parameters_path = parameters_path
    self.horizon_length = rospy.get_param(node_name + "/horizon_length")
    self.save_trajectory = rospy.get_param(node_name + "/save_trajectory")
    self.distance_threshold = rospy.get_param(node_name + "/distance_threshold")
    self.horizon_length = rospy.get_param(node_name + "/horizon_length")
...
#    rospy.logwarn("Drone || Initialization.")
#    publisher = rospy.Publisher("/hummingbird/agiros_pilot/enable", Bool, queue_size=1)
#    wait_connection(publisher, rospy.Rate(200))
#    msg = Bool()
#    msg.data = True
...
#    pose.pose.orientation.z = initial_pose.orientation.z
#    pose.pose.orientation.w = initial_pose.orientation.w
#    publisher = rospy.Publisher("/hummingbird/agiros_pilot/go_to_pose", PoseStamped, queue_size=1)
#    wait_connection(publisher, rospy.Rate(200))
#    publisher.publish(pose)
#    rospy.rostime.wallsleep(0.5)
#    while not self.is_hovering:
#      rospy.logwarn_throttle(1.0, "Drone || Waiting to enter hover state.")
#      rospy.rostime.wallsleep(0.1)
#    rospy.logwarn("Drone || Moved to initial position.")

#    rospy.logwarn("Drone || Publishing trajectory.")
#    wait_connection(self.trajectory_publisher, rospy.Rate(200))
#    trajectory_copy = copy.deepcopy(self.trajectory)
#    trajectory_copy.header.stamp = rospy.Time.now()
#    current_time = rospy.get_time()
#    for point in trajectory_copy.points:
#      point.state.t += current_time
#    self.trajectory_publisher.publish(trajectory_copy)
#    while self.is_hovering:
#      rospy.logwarn_throttle(1.0, "Drone || Waiting to leave hover state.")
#      rospy.rostime.wallsleep(0.1)
#    rospy.logwarn("Drone || Moved to final position.")
#    time.sleep(10.0)

#    rospy.logwarn("Drone || Reset.")
#    self.off_publisher.publish(Empty())
#    time.sleep(1.0)
#    self.reset_publisher.publish(Empty())
#    time.sleep(2.0)
...
#      rospy.logwarn_throttle(1.0, "Drone || Waiting to enter hover state.")
#      rospy.rostime.wallsleep(0.1)
#    rospy.logwarn("Drone || Initial hovering.")
